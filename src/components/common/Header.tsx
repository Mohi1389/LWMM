      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-6 lg:gap-8">
          <button
            ...
            className="flex items-center focus:outline-none transition-transform hover:scale-[1.02]"
            id="header-logo-btn"
          >
            <Logo size="md" />
          </button>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {user && (
            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-800/80 rounded-xl p-1 px-2.5">
              {/* Streak */}
              <div
                className="flex items-center gap-1 text-xs font-bold text-amber-600 dark:text-amber-400"
                title={`${user.streak} روز تمرین پیاپی`}
                id="streak-badge"
              >
          ...
          {/* Placement Test Alert (if level is unknown) */}
          {user?.englishLevel === 'unknown' && (
            <button
              onClick={openPlacementModal}
              id="header-placement-btn"
              className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-500 hover:bg-amber-600 text-white shadow-sm transition-transform active:scale-95 animate-bounce"
            >
          ...
          <div className="relative">
            ...
            {isLangMenuOpen && (
              <div
                ...
                } mt-2 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 py-1.5 z-50`}
              >
                <button
                  onClick={() => {
                    setLanguage('fa');
            ...
            <div className="relative">
              ...
              {isNotifOpen && (
                <div
                  className={`absolute ${
                    dir === 'rtl' ? 'left-0' : 'right-0'
                  } mt-2 w-80 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-3 z-50`}
                >
          ...
          {user ? (
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-sky-500 to-coral-500 text-white flex items-center justify-center font-bold text-xs shadow-sm">
              ...
              {isUserMenuOpen && (
                <div
                  className={`absolute ${
                    dir === 'rtl' ? 'left-0' : 'right-0'
                  } mt-2 w-52 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 py-1.5 z-50`}
                >
                  <button
                    onClick={() => {
            ...
            <button
              ...
              id="header-login-btn"
              className="px-4 py-2 rounded-xl text-xs font-bold bg-sky-600 hover:bg-sky-700 text-white shadow-sm shadow-sky-600/20 transition-all hover:shadow active:scale-95"
            >
